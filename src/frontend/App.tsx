...
function App() {
  ...
  return (
    <div className="fixed inset-0 pointer-events-none" style={{ fontFamily: "SpaceGrotesk", backgroundColor: getColorWithOpacity(baseColor, 0.5) }}>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center pointer-events-auto z-50">
          ...
          >
            ...
            <div className="space-y-8">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-200 mb-3">Max bets per column</label>
                <input
                  type="number"
                  min="1" 
                  max="10"
      ...
      {/* Betting Tips Box */}
      <div className="pointer-events-auto inline-block fixed top-6 left-6 min-w-80 max-w-7xl z-10">
        <div 
        ...
        >
          ...
          <div className="grid gap-2 justify-center overflow-x-auto max-h-[80vh] grid-flow-col auto-cols-max divide-y divide-gray-700/30">
            {betColumns.map((column: Bet[], columnIndex: number) => (
              <div key={columnIndex} className="flex flex-col min-w-80">
                {column.map((bet) => (
                    <motion.div
